...
export async function downloadArtifact(token_id: BigNumber, sizeLimit: number, polygonLimit: number, randomGateway: boolean = false): Promise<FileWithMetadata> {
    ...
    let cachedBuf = await loadFromDBCache(itemMetadata.artifactUri)
    if(!cachedBuf) {
        ...
        saveToDBCache(itemMetadata.artifactUri, cachedBuf);
    }
...
function saveToDBCache(artifactUri: string, file: ArrayBuffer) {
    if (Metadata.Storage instanceof DatabaseStorage) {
        const db = Metadata.Storage.db;
...
async function loadFromDBCache(artifactUri: string): Promise<ArrayBuffer | undefined> {
    if (Metadata.Storage instanceof DatabaseStorage) {
        const db = Metadata.Storage.db;
