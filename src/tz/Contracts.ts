...
export class Contracts {
    ...
    public async getPlaceSeqNum(walletProvider: ITezosWalletProvider, place_key: PlaceKey): Promise<PlaceSequenceNumbers> {
        ...
        const result = await current_world.contractViews.get_place_seqnum({place_key: {
            id: place_key.id, fa2: place_key.fa2}}).executeView({ viewCaller: current_world.address });
